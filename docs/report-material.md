...
| `routes/todos.rs` | 5 个 CRUD handler：获取全部、创建、更新、删除、切换完成状态 |
| `models/reminder.rs` | 定义 `Reminder` 结构体、`ReminderLevel` 枚举（Info/Warning/Danger） |
| `reminders/mod.rs` | 提醒规则引擎：due_date 检查、高优先级检查、长时间未完成检查 |
| `routes/reminders.rs` | GET /api/reminders handler，调用提醒引擎返回 JSON |

**前端模块（frontend/）：**

| 文件 | 职责 |
|------|------|
| `index.html` | 页面骨架：标题区、统计面板、添加任务区、优先级选择、日期输入、搜索框、筛选按钮、排序下拉菜单、提醒面板、导出下拉菜单、任务列表 |
| `styles.css` | 暗色/浅色双主题：CSS Variables、毛玻璃卡片、按钮组、优先级标签、下拉菜单、提醒面板三色样式、响应式适配 |
| `app.js` | 核心逻辑：fetch 封装、CRUD 操作、DOM 渲染、编辑模式、搜索筛选、排序、主题切换、JSON/CSV 导出、下拉菜单交互、提醒面板、浏览器通知 |
...
pub struct Todo {
    ...
    pub created_at: DateTime<Utc>,   // 创建时间，数据库 TIMESTAMP 类型
}
...
1. `loadTodos()` 末尾触发 `loadReminders()`
2. `fetch GET /api/reminders` 发送请求
3. Axum `get_reminders` handler 调用 `reminders::generate_reminders(&pool)`
4. 规则引擎查询 `SELECT ... FROM todos WHERE completed = FALSE`
5. 对每条未完成 Todo 依次运行 3 条检查规则（due_date / high_priority / stale）
6. 收集所有匹配的 Reminder，按 severity (danger → warning → info) 排序
7. 返回 JSON → 前端 `renderReminders()` 渲染面板
8. `sendBrowserNotifications()` 过滤 danger/warning，通过 Notification API 发送系统通知
...
| 任务排序 | ✅ 已完成 | 默认/优先级/截止日期/完成状态，下拉菜单切换 |
| 深色/浅色主题切换 | ✅ 已完成 | CSS Variables + data-theme + localStorage 持久化 |
| JSON/CSV 导出 | ✅ 已完成 | Blob + `<a>.download`，JSON 原始备份，CSV 友好格式 |
| 下拉菜单 UI | ✅ 已完成 | 导出/排序收纳为单个按钮 + 展开菜单，点击外部关闭 |
| 智能提醒后端 | ✅ 已完成 | Rust 规则引擎：截止日期/高优先级/长时间未完成 |
| 前端提醒面板 | ✅ 已完成 | danger/warning/info 分级 + 图标 + 三色样式 |
| 浏览器系统通知 | ✅ 已完成 | Notification API + sessionStorage 防重复 |
